...
export async function buildProductServer(options: ProductServerOptions): Promise<FastifyInstance> {
  ...
  app.get("/v1/agents/:id", api(async (principal,request,reply)=>{requireAnyScope(principal,["actions:read","integrations:read"]);return found(reply,await options.repository.agentDetail(principal,routeId(request)));},options.repository));
