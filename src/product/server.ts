...
export async function buildProductServer(options: ProductServerOptions): Promise<FastifyInstance> {
  ...
  app.get("/v1/actions/:id/review-options", api(async (principal,request)=>{requireScope(principal,"actions:read");return options.repository.humanReviewOptions(principal,routeId(request));},options.repository));
