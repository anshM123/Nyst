...
export async function buildProductServer(options: ProductServerOptions): Promise<FastifyInstance> {
  ...
  app.post("/v1/integrations/:provider/credential", api(async (principal, request) => {
    ...
    // `store` proves ownership: it just wrote this credential for THIS scope.
    // Passed anyway so the guard is exercised on the path that uses it most.
    await options.repository.reconnectIntegration(principal, provider, stored.credential_ref, store);
